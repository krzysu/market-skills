...
6. **Pick logic (top-3 cap)** — if any idea has `met_bar: true` AND `cooldown_ok: true`, **dedup by ticker first** (same ticker can fire multiple strategies — e.g. accum-swing + liquidity-sweep both long ETH with near-identical levels from the same 4h structure — which would otherwise let one asset eat multiple slots, worked 2026-08-19: LINK + ETH + ETH). Keep the highest-conviction idea per ticker, tie-break by R:R desc, then pick the top 3 by R:R descending. Tie-break after dedup: conviction desc, source priority (tier 1+2 > swing_shortlist > coingecko_movers > smart_money > hl_narrative), tier (1 before 2), ticker alphabetical. Set `picked: true` on each of the top 3 and `picked: false` on the rest. Each picked idea gets `suggested_size_eur` from its source's cap (tier 1+2: EUR 200; swing + coingecko + smart money: EUR 100; hl_narrative: EUR 50 perp notional), then adjusted by the track-record multiplier (step 7).
