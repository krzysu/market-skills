...
1. **No auto-execution.** The pick is a recommendation. The operator reads it, decides, places via `execution-kraken-spot submit` or `execution-kraken-perps submit` manually.
...
- **HYPE bleeds across the scan universe.** 11 closed ideas, 18% hit rate, -0.28% avg return — worst-performing tracked ticker. Perp-DEX scaling rule keeps most HYPE at conv=1/2 so the conviction floor catches it, but it still logs every tick. Remove from tier_1 scan (or set `tracking_only: true`); don't lower the bar.
...
- **Pick rate < 1/day (2% of ideas, 0.3/day avg).** Don't lower the bar. Two paths: (a) tighten universe per the HYPE pitfall, (b) expand to sources with stricter gates (≥4 conviction). The "silent 3+ ticks → expand universe" rule in When-to-use is the canonical path (b).
