...
- Mirror venue **cash** (`ZEUR`/`ZUSD`) into the ledger as a balance you keep in sync with the venue. The **single** sanctioned exception is the derived working-capital cash row below — everything else cash-shaped stays out. Sell proceeds still never get their own rows: the cash row's derived balance absorbs them automatically.

**The one sanctioned exception — a working-capital cash row derived from the ledger's own transactions.** Book the initial allocation once (e.g. `BUY kraken:EUR 1000 @ 1.0`) and never edit that row again: from then on its value is **derived** from the portfolio's transaction stream — cash-asset BUYs (deposits) add `qty*price`, non-cash BUYs subtract `qty*price + fee`, non-cash SELLs add `qty*price - fee`, and a SELL of the cash asset itself is a withdrawal. The derivation is scoped to the row's own lifetime: transactions booked **before the row first exists** are not charged to it (they were funded from capital the ledger does not model). Booking the initial allocation **first** — the documented convention — keeps the row equal to the simple sum over the whole transaction stream. The row is **not a venue mirror** — the ledger records **decisions** while the venue records **reality**, and the two must **never** be reconciled to each other (the derivation reads the ledger alone, never a venue API; staking drift and venue-side deposits/withdrawals are not represented). Never tune the row to match `kraken balance`, and never insert a cash row because a key appeared in a balance output. A **negative** derived balance is permitted and reported as-is: it means the book deployed more than its cash row.

**The asset key must be `kraken:EUR`, not `ZEUR`.** `risk-engine` populates `cash_available` by comparing `_strip_prefix(asset).upper()` against the portfolio's `base_ccy`, so only the prefixed form matches; a bare `ZEUR` key leaves `cash_available` at 0 and the row is silently ignored.

Why the row exists at all: without a cash key the ledger has no liquidity figure, and any `insufficient_funds` check has nothing to read. With `kraken:EUR` present the policy is live — the derived balance feeds `risk-engine`'s `cash_available` directly, so funds checks track the real deployable amount: opening a position reduces it, closing one restores it (minus fees and any realized loss).

> **One-time peak re-baseline.** `portfolios.peak_value` recorded under the old frozen-notional model double-counted closed swings (the frozen cash notional and the position notional were both on the books), so every close registered as drawdown. On the first `compute_portfolio_drawdown` call for a portfolio whose `portfolios.peak_model` marker is unset, the stale peak is reset to 0 — only if the portfolio actually has a cash row — and re-seeded from the corrected equity. This fires at most once per portfolio, requires no manual SQL (the reset is lazy on the first drawdown call), and never touches a portfolio without a cash row.
