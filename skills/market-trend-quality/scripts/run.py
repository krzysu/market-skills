#!/usr/bin/env python3
"""market-trend-quality — L2 pattern detection: assesses trend health quality."""

import sys
from datetime import UTC, datetime

from analysis.data import fetch_ohlc
from analysis.formatting import print_header, safe_parse_args
from analysis.output import (
    cache_run_result,
    emit_envelope_json,
    empty_state,
    maybe_render_home_view,
    parse_axi_flags,
    print_envelope,
    resolve_fields,
    truncate,
)
from analysis.skill_loader import load_lib_for_script

NARRATIVE_LIMIT = 120


def analyze(ticker, *, source=None, interval="1d", period="1y"):
    candles = fetch_ohlc(ticker, interval=interval, period=period, source=source)
    if not candles:
        return {"ticker": ticker, "error": "no data"}

    _lib = load_lib_for_script(__file__)
    result = _lib.analyze(candles, interval=interval, period=period)
    if "insufficient" in result.get("narrative", ""):
        return {"ticker": ticker, "error": result["narrative"]}

    now = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    provider = source or "auto-detected"

    pattern = result["pattern"]
    fired = bool(pattern.get("present")) and pattern.get("classification") is not None

    return {
        "skill": "market-trend-quality",
        "ticker": ticker,
        "timestamp": now,
        "provider": provider,
        "interval": interval,
        "period": period,
        "fired": fired,
        "classification": pattern.get("classification"),
        "confidence": pattern.get("confidence"),
        "max_confidence": pattern.get("max_confidence"),
        "type": pattern.get("type"),
        "signals": result["signals"],
        "input_scores": result["input_scores"],
        "narrative": result["narrative"],
    }


def _help_lines(ticker: str) -> list[str]:
    return [
        f"Run `strategy-trend-follow {ticker} --json` for the L3 idea keyed off this verdict",
        "Pass --full for the full payload or --fields=<csv> to project",
    ]


def main():
    fields_arg, full, toon, filtered_argv = parse_axi_flags(sys.argv[1:])
    ticker, json_mode, source, interval, period = safe_parse_args(filtered_argv)
    if maybe_render_home_view(__file__, ticker, json_mode):
        return
    result = analyze(ticker, source=source, interval=interval, period=period)
    cache_run_result(__file__, result)

    if json_mode:
        if "error" in result:
            print_envelope(empty_state(errors=[result["error"]], help=_help_lines(ticker or "TICKER")))
            return
        fields = resolve_fields(
            fields_arg,
            full=full,
            default=["ticker", "fired", "classification", "confidence"],
        )
        emit_envelope_json(
            result,
            count=1,
            help=_help_lines(ticker),
            fields=fields, toon=toon,)
        return

    if "error" in result:
        print(f"  {ticker}: {result['error']}")
        return

    pat = result["pattern"] if "pattern" in result else result
    fired = result.get("fired", bool(pat.get("present")))
    cls = pat.get("classification") or result.get("classification")
    conf = pat.get("confidence", result.get("confidence"))
    maxc = pat.get("max_confidence", result.get("max_confidence", 5))
    sigs = result.get("signals", {})

    print_header(f"TREND QUALITY PATTERN — {ticker}")
    print(f"  {ticker}")
    print()
    print(f"    Pattern Present:  {'YES' if fired else 'NO'}  (confidence: {conf}/{maxc})")
    if cls:
        print(f"    Classification:   {cls}")
    if result.get("type"):
        print(f"    Type:             {result['type']}")
    print()
    if sigs:
        print("  Sub-Signals:")
        for name, sig in sigs.items():
            check = "\u2713" if sig.get("present") else "\u2717"
            print(f"    {check} {name:<30s}  (w={sig.get('weight')})")
        print()
    print(f"  {truncate(result.get('narrative', ''), limit=NARRATIVE_LIMIT)}")
    print()


if __name__ == "__main__":
    main()
