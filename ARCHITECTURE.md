...
- Conviction gate → `lookup_min_conviction(strategy, ticker, interval) -> int` — per-(strategy, ticker, interval) minimum-conviction-to-emit floor for L3 ideas. Lives in `analysis/conviction_thresholds.py`; `MIN_CONVICTION_TO_EMIT_BY_STRATEGY` holds the overrides (loaded from `$MARKET_SKILLS_CONVICTION_THRESHOLDS_PATH` so per-tuner evidence stays outside the open-source repo), `GLOBAL_MIN_CONVICTION_TO_EMIT = 1` is the no-op default. The table is keyed `provider:ticker` (as written by the backtest pipeline); the lookup resolves the query ticker notation on the read side — bare symbol, `provider:symbol`, and `BASE-QUOTE`/`BASE/QUOTE` forms bind to the same stored key, and unmatched combinations fall through to the global default with a counted, debug-logged fall-through (`fallthrough_stats()`). Read by every L3 strategy at the end of `analyze()` (via `finalize_ideas`) to drop ideas whose conviction is below the threshold before emit.
...
| Breadth    | Watchlist basket + benchmark daily OHLC via `fetch_ohlc` (kraken by default) | Breadth payload (`pct_beating`, `regime`, leaders/laggards) | Stateless | **Built** — pct of basket beating BTC, narrate-only, not wired into L3/risk ([ADR-0010](./docs/adr/0010-cross-sectional-breadth-is-not-an-l2.md)) |
...
| Monitoring | Watches JSON + Portfolio + venue closed-orders | Alert events                  | Per-watch state                                                                      | **Built** (position-watchdog, manual confirm only)                                            |
