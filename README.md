...
| [market-breadth](./skills/market-breadth/SKILL.md) | Cross-sectional altcoin breadth: share of a watchlist basket outperforming BTC over a rolling N-day window (`--window-days=N`, default 7). Ticker-agnostic CLI: `uv run skills/market-breadth/scripts/run.py --json`. Universe resolved from a `market-watchlist` basket (`--basket`, default `tier_1`, falling back to the first non-empty watchlist basket when absent — substitution disclosed in `errors[]`); benchmark resolves from the same registry (`--benchmark`, default `btc`, excluded from the member set). Returns `pct_beating`, `regime` (`alt_rotation` / `mixed` / `btc_led` — uncalibrated bands, trust `pct_beating`), `leaders` / `laggards`. Narrate-only; not wired into L3 conviction or risk sizing ([ADR-0010](./docs/adr/0010-cross-sectional-breadth-is-not-an-l2.md)). |
