"""Per-fix fixture for strategy-mean-reversion's per-(ticker, interval)
...
mean-reversion specifics. Before ``6th`` this strategy emitted every
...
def _load_lib():
    lib_path = os.path.join(os.path.dirname(__file__), "..", "skills", "strategy-mean-reversion", "lib.py")
...
def _make_candles(n=250, base=99.5, half_range=1.5, lock_close=99.7, seed=42):
    """Build [ts, open, high, low, close, volume] candles with the final
    close pinned via ``lock_close`` so price sits near the canned support
    (``99.5``) and the long mean-reversion entry condition
    (``price <= support * 1.02``) fires deterministically.
...
def _canned(vol_regime: str = "NORMAL"):
    """Build canned L2 stubs that fire one long mean-reversion idea.

    RSI oversold (``25``) at support (``99.5``) with a non-LOW volatility
    regime yields ``conviction = 3 if low_vol else 2`` = ``2`` — a
    low-conviction idea that a raised threshold (``>= 4``) must drop.
    ``market-valuation`` is omitted so the CAPE-valuation hook is a no-op.
    """
    return {
        "market-rsi": _stub("R", {"rsi_14": 25}),
        "market-s-r": _stub("S", {"nearest_support": 99.5, "nearest_resistance": None}),
        "market-volatility": _stub("V", {"regime": vol_regime}),
    }
...
_STRATEGY = "strategy-mean-reversion"
...
class TestGateEndToEnd:
    ...
    def test_default_global_one_preserves_legacy_emit_all(self, monkeypatch):
        ...
        assert result["ideas"][0]["conviction"] == 2, (
            f"Stub produces conviction=2 (non-LOW vol regime); got {result['ideas'][0]['conviction']}"
        )

    def test_raised_threshold_drops_low_conviction_idea(self, monkeypatch):
        """A per-(ticker, interval) override of MIN=4 must drop the
        conviction=2 idea end-to-end."""
        _install(monkeypatch, _canned())
        mod = _load_lib()
        with _patched_threshold(_STRATEGY, "hl:HIGHGATE", "1d", 4):
            result = mod.analyze(_make_candles(), ticker="hl:HIGHGATE", interval="1d", period="1y")
        assert result["ideas"] == [], f"Override MIN=4 must drop conv=2 idea; got {result['ideas']}"

    def test_override_shifts_pipeline_vs_unknown_ticker(self, monkeypatch):
        """A per-(ticker, interval) override of MIN=4 must change the lib's
        ...
        with _patched_threshold(_STRATEGY, "hl:OVERRIDE", "4h", 4):
        ...
        assert r_override["ideas"] == [], f"MIN=4 should drop conv=2 idea; got {r_override['ideas']}"
