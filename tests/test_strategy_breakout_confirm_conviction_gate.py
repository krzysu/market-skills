"""Per-fix fixture for strategy-breakout-confirm's per-(ticker, interval)
...
breakout-confirm specifics. Before ``6th`` this strategy emitted every
...
def _make_candles(n=250, base=100.0, half_range=1.5, seed=42):
    ...
    ``half_range=1.5`` (= 3% per candle) ensures ATR-derived stops clear
    the 2% swing-minimum floor so the surviving idea is not dropped by the
    stop-distance filter before the conviction gate runs.
    """
    import random
...
def _canned(bo_conf: int = 3, squeeze_signal: str = "NEUTRAL"):
    """Build canned L2 stubs that fire one long breakout-confirm idea.

    ``bo_direction='bull'`` + ``volume_ratio > 1.2`` + ``obv_trend='rising'``
    unlocks the long branch (the squeeze-OR gate is satisfied by OBV). With
    ``bo_conf=3`` and a non-bullish ``squeeze_signal`` the L3 conviction
    formula is ``min(5, 3 + 0) = 3`` — a low-conviction idea that a raised
    threshold (``>= 4``) must drop.
    """
    return {
        "market-breakout": _stub(
            "BO",
            {
                "pattern": {
                    "present": True,
        ...
        "market-squeeze": _stub("Z", {"signal": squeeze_signal}),
    }
...
_STRATEGY = "strategy-breakout-confirm"
...
class TestGateEndToEnd:
    ...
    def test_default_global_one_preserves_legacy_emit_all(self, monkeypatch):
        ...
        assert result["ideas"][0]["conviction"] == 3, (
            f"Stub produces conviction=3 (bo 3 + non-bullish squeeze 0); got {result['ideas'][0]['conviction']}"
        )
