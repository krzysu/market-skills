...
class TestWeightedSumArithmetic:
    ...
    mirrors the bug-scan Shape #1 trigger. See TestAbsentWithSubsRegression for behavioral tests.
    """
    ...
    def test_volume_plus_retest_above_030(self):
        ...
        by the pre-fix ``ratio >= 0.40`` threshold. Post-fix the count + weight trigger fires this case.
        """
...
class TestAbsentWithSubsRegression:
    ...
    ghost (absent-with-subs). Post-fix the L2 trigger is
    ``n_present >= 2 AND weighted_sum > 0.30``.
    """
