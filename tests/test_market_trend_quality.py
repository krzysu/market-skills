...
class TestThreeSubSignalFallback:
    ...
    The ``weighted_sum > 0.30`` threshold mirrors the bug-scan Shape #1 trigger
    (skills/bug-scan/SKILL.md) and the ALGO 4h liquidity-sweep fix
    (BUG-2026-06-24-01). It replaces the pre-fix ``abs(signed_score) >= 0.50``
    gate which silently dropped cases where opposing signs dilute signed_score
    while the directional signal is still strong (see
    ``test_three_sub_opposing_signs_wsum_above_030_classifies`` below — VVV 4h
    trend-quality, BUG-2026-06-24-02).
    ...
    def test_three_sub_count_gate_protects_two_subs(self, monkeypatch):
        ...
        trigger the 3-sub WEAKENING path even at high weighted_sum.
        ...
        silently over-fire on every 2-sub borderline case (the bug-scan
        Shape #1 detector would never trigger because l2_fired would be True).
        ...
        None of the gates fire (signed_score < 0.75, trend_score=0, 3-sub
        count-gated, 4-sub unreachable). Classification must stay None.
        """
        import analysis.skill_loader as sl
        ...
        # No gate fires for this shape (signed_score 0.50 < 0.75, trend_score=0,
        # 3-sub count==3 not met, 4-sub count>=4 not met). Consistent ghost:
        # classification=None, present=False.
        assert result["pattern"]["classification"] is None, (
            f"2-sub case at signed_score=0.50 / wsum=0.50 must NOT classify "
            f"(3-sub branch is count-gated). Got "
            f"classification={result['pattern']['classification']!r} with "
            f"present_count={present_count}: {result['signals']}"
        )
        assert result["pattern"]["present"] is False

    def test_three_sub_opposing_signs_wsum_above_030_classifies(self, monkeypatch):
        """Regression for BUG-2026-06-24-02 (VVV 4h market-trend-quality,
        ...
        Post-fix the trigger uses ``weighted_sum > 0.30`` mirroring the
        bug-scan Shape #1 trigger and the ALGO 4h liquidity-sweep fix
        (BUG-2026-06-24-01), so this case classifies as WEAKENING.
        """
        # ema: present (FULL_BEAR)         contribution -0.25
        # hh_hl: present (hh=T, hl=T)     contribution +0.25
        ...
        assert result["pattern"]["classification"] == "WEAKENING", (
            f"BUG-2026-06-24-02 (VVV 4h) regression: 3 subs with opposing "
            f"signs (wsum=0.70, signed_score=0.20) must classify as WEAKENING. "
