"""Per-fix fixture for strategy-accumulation-swing's per-(ticker, interval)
...
accumulation-swing specifics. Before ``6th`` this strategy emitted every
...
def _load_lib():
    lib_path = os.path.join(os.path.dirname(__file__), "..", "skills", "strategy-accumulation-swing", "lib.py")
    spec = importlib.util.spec_from_file_location("strategy_accumulation_swing_lib", lib_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def _make_candles(n=250, base=100.0, half_range=1.5, seed=7):
...
def _canned(accum_conf: int = 3, tq_classification: str = "WEAKENING"):
    """Build canned L2 stubs that fire one long idea.

    With ``accum_conf=3`` and ``tq_classification="WEAKENING"`` the L3
    conviction formula is ``min(5, 3 + 0) = 3`` — a low-conviction idea
    that a raised threshold (``>= 4``) must drop.
    """
    return {
        ...
        "market-trend-quality": _stub(
            "TQ",
            {
                "pattern": {
                    ...
                    "confidence": 3,
                    "max_confidence": 5,
                    "classification": tq_classification,
                    "type": "TREND_QUALITY",
                },
                "narrative": "stub",
                "input_scores": {},
...
class TestGateEndToEnd:
    ...
    def test_default_global_one_preserves_legacy_emit_all(self, monkeypatch):
        ...
        assert result["ideas"][0]["conviction"] == 3, (
            f"Stub produces conviction=3 (accum 3 + WEAKENING 0); got {result['ideas'][0]['conviction']}"
        )
